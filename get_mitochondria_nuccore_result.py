...
from selenium.webdriver.edge.options import Options
def get_mitochondria_nuccore_result(fold):
    old_file_name='nuccore_result.txt'
    new_file_name='mitochondria_nuccore_result.txt'
    ...
    #
    # # 在元素中输入内容
    element.send_keys('(genome AND plants[filter] AND mitochondrion[filter] AND ( "30000"[SLEN] : "20000000"[SLEN] )) ')
